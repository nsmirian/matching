...
%     refresh 
    ...
    function out = assemble_matrix_fast(m,mat,k,quadindx)
        indx = [m(:).active]==1;
        tempindx = find(quadindx);
        fquadindex = tempindx(indx);
        for i=indx
            M = Quadrupole(k(i),m(i).effective_length/2); % include all quadrupole halfes    
            mat(:,:,fquadindex(i)) = M;
            mat(:,:,fquadindex(i)+1) = M;
        end
        out{1} = eye(6);
        for i=2:size(mat,3)
            out{i} = mat(:,:,i)*out{i-1};
        ...
        for i=1:length(m)           
            switch m(i).type
                case 'QUAD'
                    ...
                    out=cat(3,out,M,M);
                    z = [z, z(end)+m(i).effective_length/2, z(end)+m(i).effective_length];
    ...
    function out = get_matching_matrix(m,k,refpos,zlist)
        [outtemp,ztemp] = assemble_matrix_fast(m,k,refpos);
