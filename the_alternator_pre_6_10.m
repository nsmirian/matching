...
%                twiss_goal  -  matching target parameters
    ...
    matrix_orig = assemble_matrix(m,k_orig,matrices);
    ...
    options=optimset('MaxFunEvals',length(m)*1000,'MaxIter',length(m)*1000,'TolFun',1e-8);
    % get_matching_matrix_curly sollte (implizit) assemble matrix_simple
    % verwenden
    k_new = fminsearch(@(k) match_cond(twiss_back,twiss_goal,get_matching_matrix_curly(m,k,matrices,zlist),zlist),k_rand,options);

    mismatch_xy = match_cond(twiss_back,twiss_goal,get_matching_matrix_curly(m,k_new,matrices));
    
    % todo partial d mismatch / partial d k_new_i

    [matrices,z,isquad] = assemble_matrix_full(m,k_new,refpos);  % simple -- muss noch getested werden
    outtmp = assemble_matrix(m,k_new,matrices);  % mit assemble matrix_full
...
%     refresh 
    ...
    function out = assemble_matrix(m,k,mat)
    ...
    function out = get_matching_matrix(m,k,mat,zlist)
        outtemp= assemble_matrix(m,k,mat);
    ...
    function out = get_matching_matrix_curly(m,k,mat,zlist)
        % aendere funtion so dass cell von matrizen gecurled wird
        if nargin==4
            temp = get_matching_matrix(m,k,mat,zlist);
            out = [];
            for i=1:size(temp,3)
                out = cat(3,out,CurlyM(temp(:,:,i)));
            end
        else
            temp = get_matching_matrix(m,k,mat);
            out = CurlyM(temp);
        end
    end
end
